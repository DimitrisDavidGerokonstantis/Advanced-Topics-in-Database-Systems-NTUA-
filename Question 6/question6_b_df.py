...
full_dataset_df = basic_dataset_2010to2019_df_casted.union(basic_dataset_2020topresent_df_casted).filter((col("LAT")!=0)&(col("LON")!=0))
...
.groupBy("DIVISION").agg(count("*").alias("#"),avg("distance").alias("average_distance")).orderBy(col("#").desc()).select("DIVISION","average_distance","#").show()
